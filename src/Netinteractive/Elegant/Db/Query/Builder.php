...
class Builder implements BuilderInterface
{
    ...
    /**
     * List of QueryBuilder objects to build CTE statement
     * @var array
     */
    protected $cte = array();
    ...
    /**
     * Binds QueryObject to $alias and later makes SQL CTE statement
     * @param \Netinteractive\Elegant\Db\Query\Builder $query
     * @param $alias
     * @return $this
     */
    public function addCTE(Builder $query, $alias)
    {
        $this->cte[$alias] = $query;
     ...
     * @return array
     */
    public function getCTE($alias = null)
    {
        if (!empty($alias) && isSet($this->cte[$alias])){
    ...
    protected function prepareQuery()
    {
        ...
        #Here we build CTE statment (PostgreSQL)
        $cte = '';
        $max = count($this->cte);
        ...
        if ($max) {
            ...
            foreach ($this->cte as $alias => $cteQuery) {
                $cte .= "$alias AS (";
                $cte .= $this->grammar->compileSelect($cteQuery);
                $cte .= ')';
